...
var title = "Runtime Function of Simple Recursive Nim Algorithm";
var x = d3.scaleLinear().range([0, width]); // linearly scaled Y axis

var y = d3.scaleLinear().range([height, 0]); // for logarithmically scaled Y -axis

var y = d3.scaleLog().base(10).domain([1, 1000]).range([height, 0]); // create a horizontal and vertical line of graph
...
var svg = d3.select("body").append("svg").attr("width", 600).attr("height", 400).append("g").attr("transform", "translate( ".concat(margin.left, " , ").concat(margin.top, ")")); // add the label in x axis and y axis
...
d3.csv("files/timeRecord2.csv", function (err, value) {
  ...
  x.domain([0, d3.max(value, function (d) {
    return d.number;
  })]);
  y.domain(d3.extent(value, function (d) {
    console.log(_typeof(d.executionTime));
...
//# sourceMappingURL=graphPlot.dev.js.map
